...
const SCENE_LAYOUT_SETTING = "sceneBuilderLayout";
...
const SCENE_CATEGORY_IDS = SCENE_CATEGORY_DEFINITIONS.map((category) => category.id);
const SCENE_CARD_IDS = SCENE_CARD_DEFINITIONS.map((card) => card.id);
...
function getQuickAccessCards(layout) {
  ...
  const mostUsedIds = Object.entries(layout.useCounts)
    .filter(([cardId, count]) => SCENE_CARD_IDS.includes(cardId) && count > 0 && !favoriteIds.includes(cardId))
...
function normalizeSceneLayout(layout) {
  ...
  const categoryOrder = Array.from(new Set(Array.isArray(input.categoryOrder) ? input.categoryOrder : []))
    .filter((categoryId) => SCENE_CATEGORY_IDS.includes(categoryId));
  defaults.categoryOrder.forEach((categoryId) => {
    if (!categoryOrder.includes(categoryId)) categoryOrder.push(categoryId);
  });
...
export default class CPRRolltableDashboard extends FormApplication {
  ...
  async _toggleCardFavorite(cardId) {
    if (!cardId || !SCENE_CARD_IDS.includes(cardId)) return;
    const layout = this._getSceneLayout();
    if (layout.favoriteCardIds.includes(cardId)) layout.favoriteCardIds = layout.favoriteCardIds.filter((id) => id !== cardId);
    else layout.favoriteCardIds.unshift(cardId);
    await this._saveSceneLayout(layout);
  }

  async _toggleCategoryVisibility(categoryId) {
    if (!categoryId || !SCENE_CATEGORY_IDS.includes(categoryId)) return;
    const layout = this._getSceneLayout();
    if (layout.hiddenCategoryIds.includes(categoryId)) layout.hiddenCategoryIds = layout.hiddenCategoryIds.filter((id) => id !== categoryId);
    else layout.hiddenCategoryIds.push(categoryId);
    await this._saveSceneLayout(layout);
  }
  ...
  async _runSceneCard(cardId, cardElement = null) {
    ...
    if (card.action === "nightMarket") return this._generateNightMarket();
    if (card.action === "merchant") return this._generateMerchantChat(this._getScopedCardValue(cardElement, "merchantCategory"));
    if (card.action === "encounter") return this._generateEncounter(this._getScopedCardValue(cardElement, "encounterPeriod"));
    if (card.action === "roleHustle") return this._generateHustle(this._getScopedCardValue(cardElement, "roleHustleGenerator"), this._getScopedCardValue(cardElement, "roleHustleRankBand"));
    if (card.action === "namedChain") return this._generateNamedDesktopChain(card.groupLabel, card.chainLabel);
    if (card.action === "vendit") return this._generateVendit();
  }
  ...
  _handleSceneDragStart(event) {
    ...
    const payload = target.classList.contains("cpr-action-card")
      ? { type: "card", id: target.dataset.cardId, categoryId: target.closest(".cpr-scene-category")?.dataset.categoryId }
      : { type: "category", id: target.dataset.categoryId };
